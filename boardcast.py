...
async def _broadcast(text_ru, text_en, text_ro, photo_url=None):
    from bot.main import bot  # ленивый импорт внутри функции, чтобы избежать цикла при старте
...
def send_broadcast(text_ru, text_en, text_ro, photo_url=None):
    """Вызывает корутину рассылки в event loop бота"""
    from bot.main import dp
    loop = dp.loop if hasattr(dp, 'loop') else asyncio.get_event_loop()
    if loop.is_running():
        asyncio.create_task(_broadcast(text_ru, text_en, text_ro, photo_url))
    else:
        loop.run_until_complete(_broadcast(text_ru, text_en, text_ro, photo_url))
